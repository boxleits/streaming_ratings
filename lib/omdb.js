...
/**
 * Splits OMDb cache entries due for a check into three priority tiers:
 *
 *  - `neverChecked`   - rt still "TODO": a genuine first-time gap.
 *  - `dueForRefresh`  - previously rated, now stale: a nice-to-have
 *                       background re-check, not a gap in coverage. Covers
 *                       both the shared OMDb staleness flag (`needsRefresh`)
 *                       and RT's own, shorter-interval one
 *                       (`rtNeedsRefresh`) - either way the entry already
 *                       shows a value and is only due a refresh.
 *  - `metacriticOnly` - the RT score is known (typically from the RT
 *                       scraper) but OMDb was rate-limited before it could
 *                       supply Metacritic. Lowest priority: the visible RT
 *                       column is already correct, only Metacritic is owed.
 *
 * Keeping the tiers separate lets the engine tell whether a quota hit still
 * blocks real coverage or only delays something optional (see server.js's
 * choice between the "waiting_for_limit_reset" and "stale_refresh_pending"
 * statuses), and lets it skip re-scraping RT for entries whose RT is
 * already fresh.
 */
export function splitPendingOmdbIds(entries) {
  const neverChecked = [];
  const dueForRefresh = [];
  const metacriticOnly = [];
  for (const [id, entry] of Object.entries(entries || {})) {
    if (entry.rt === "TODO") neverChecked.push(id);
    else if (entry.needsRefresh || entry.rtNeedsRefresh) dueForRefresh.push(id);
    else if (entry.metacriticPending) metacriticOnly.push(id);
  }
  return { neverChecked, dueForRefresh, metacriticOnly };
}

/**
 * Picks which OMDb cache entries are due for a check, ordered so movies
 * that have NEVER been rated come first, then stale refreshes, then
 * entries that only still owe a Metacritic score - see splitPendingOmdbIds
 * for why the priority is this way around.
 */
