...
// with ratings - currently OMDb (Metacritic, and RT as a fallback) and the
// Rotten Tomatoes scraper (tomatometer). The secondaries are deliberately
// peers: each keeps its own cache, its own refresh interval, its own status
// and its own manual trigger, and none of them can block, stall or
// invalidate another. This module holds the parts that are identical for
// all of them - scheduling (what's due for a check) and the merge into a
// single view - so neither source has to know about the other.
//
 ...
 * shows. The RT scraper wins the RT column whenever it has actually been
 * checked - it reads the score off Rotten Tomatoes itself, whereas OMDb's
 * RT figure is a second-hand copy that is missing for many titles - but a
 * movie the scraper hasn't reached (or that has no RT page) still falls
 * back to OMDb's, so enabling the scraper never removes data.
 *
 * "TODO" means no source has checked yet; `null` means checked, no rating
 * on file.
 */
export function mergeRatingView(omdbEntry, rtEntry) {
  ...
  return {
    ...
    metacritic: omdb.checkedAt ? omdb.metacritic : "TODO",
    ...
    ratingNeedsRefresh: Boolean(rt.needsRefresh || omdb.needsRefresh),
  };
}
